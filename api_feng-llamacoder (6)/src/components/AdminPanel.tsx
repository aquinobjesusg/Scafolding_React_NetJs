...
export function AdminPanel({ onExit, token, user }: { onExit: () => void; token: string; user: any }) {
  const [collapsed, setCollapsed] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);
  const [openMenus, setOpenMenus] = useState<Record<string, boolean>>({});
  const [data, setData] = useState<Record<string, CrudItem[]>>({});
  ...
  const [profileData, setProfileData] = useState({ nombre: user?.name || "Admin User", email: user?.email || "admin@fengoffice.com", telefono: user?.phone || "+123 456 7890", bio: user?.bio || "Administrador del sistema." });
  ...
  const [stats, setStats] = useState<{ barData: any[]; pieData: any[]; tableData: any[] }>({ barData: [], pieData: [], tableData: [] });
  ...
  const fetchData = async (section: string) => {
    ...
    try {
      if (section === "facturas") {
        const result = await api.facturas.list(token);
        setFacturas(result);
      } else if (section !== "estadisticas" && section !== "perfil") {
        const result = await api.crud.list(section, token);
        setData(prev => ({ ...prev, [section]: result }));
      } else if (section === "estadisticas") {
        const result = await api.stats.get(token);
        setStats(result);
      } else if (section === "perfil") {
        const result = await api.profile.get(token);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        const updated = await api.crud.update(activeSection, editingItem.id, formData, token);
        setData(prev => ({ ...prev, [activeSection]: prev[activeSection].map(i => i.id === editingItem.id ? updated : i) }));
      } else {
        const created = await api.crud.create(activeSection, formData, token);
        setData(prev => ({ ...prev, [activeSection]: [...(prev[activeSection] || []), created] }));
      }
      setIsModalOpen(false);
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await api.crud.delete(activeSection, id, token);
  ...
  const handleFacturaSave = async () => {
    setLoading(true);
    try {
      if (editingFactura) {
        const updated = await api.facturas.update(editingFactura.id, facturaForm, token);
        setFacturas(facturas.map(f => f.id === editingFactura.id ? updated : f));
      } else {
        const created = await api.facturas.create(facturaForm, token);
  ...
  const handleDeleteFactura = async (id: string) => {
    try {
      await api.facturas.delete(id, token);
  ...
  const handleProfileSave = async () => {
    setLoading(true);
    try {
      await api.profile.update({ ...profileData, image: profileImage }, token);
    } catch (err: any) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const handlePasswordChange = async () => {
    setLoading(true);
    try {
      await api.profile.changePassword({ actual: passwordData.actual, nueva: passwordData.nueva }, token);
  ...
  const renderContent = () => {
    ...
    if (error) return <div className="flex h-64 items-center justify-center text-rose-600"><AlertCircle className="mr-2 h-6 w-6" /> {error}</div>;
        ...
        return (
          <Card className="border-slate-200 bg-white shadow-sm">
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <RechartsPieChart>
                  <Pie data={stats.pieData} cx="50%" cy="50%" labelLine={false} outerRadius={150} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
          ...
          <Card className="border-slate-200 bg-white shadow-sm">
            ...
            <CardContent className="space-y-6">
              <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center">
                <div className="relative">
                  <Avatar className="h-24 w-24 border-4 border-slate-100 shadow-sm">
                    ...
                    <AvatarFallback className={`text-2xl ${activeTheme.color} text-white`}>{profileData.nombre.charAt(0)}</AvatarFallback>
                  </Avatar>
              ...
              <div className="flex justify-end">
                <Button onClick={handleProfileSave} className={`${activeTheme.color} ${activeTheme.hover} text-white`} disabled={loading}>
        ...
        return (
          <Card className="border-slate-200 bg-white shadow-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-end">
                <Button onClick={handlePasswordChange} className={`${activeTheme.color} ${activeTheme.hover} text-white`} disabled={loading}>
                  {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
        ...
        <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 lg:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Avatar className="h-9 w-9 border-2 border-slate-100">
              <AvatarImage src={profileImage} />
              <AvatarFallback className={`${activeTheme.color} text-white text-sm font-semibold`}>{profileData.nombre.charAt(0)}</AvatarFallback>
            </Avatar>
          </div>
        </header>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} className={`${activeTheme.color} ${activeTheme.hover} text-white`} disabled={loading}>
              {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
      ...
      <Dialog open={isFacturaModalOpen} onOpenChange={setIsFacturaModalOpen}>
        <DialogContent className="sm:max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleFacturaSave} className={`${activeTheme.color} ${activeTheme.hover} text-white`} disabled={loading}>
              {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}

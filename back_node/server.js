...
const pool = new Pool({
  user: process.env.DB_USER || 'oxdiccionario',
  host: process.env.DB_HOST || 'localhost',
  database: process.env.DB_NAME || 'oxdiccionario',
  password: process.env.DB_PASSWORD || 'oxdiccionario',
...
app.post('/registrar', async (req, res) => {
  const { nombre, apellido, email, password, usuario, idioma } = req.body;
  if (!nombre || !apellido  || !email || !password || !usuario || !idioma ) {
    return res.status(400).json({ error: 'Los campos  son requeridos' });
  }
  try {
    const result = await pool.query(
      'INSERT INTO users (nombre, apellido, email, password, usuario, inactivo, incambioclave, id_languajes) VALUES ($1, $2, $3, $4, $5, 1, 0, $6) RETURNING *',
      [nombre, apellido, email, password, usuario, idioma]
    );
    return res.status(200).json({ error:'ok' });
    //res.status(201).json(result.rows[0]);
  } catch (err) {

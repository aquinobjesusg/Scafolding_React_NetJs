# h#ttp://127.0.0.1:5000/
# h#ttps://systemsya.com/diccionario/
...
def ejecutar_envio_mail(destino, asunto, cuerpo):
    """Envía correo electrónico a través de una API"""
    
    # Configuración de la API
    API_URL = "https://systemsya.com/correo/index.php"  # Reemplaza con la URL real
    # API_KEY = "tu-api-key"  # Si la API requiere autenticación
    ...
    headers = {
        ...
        # 'Authorization': f'Bearer {API_KEY}'  # Si requiere autenticación
    }
    
    print('Enviando correo a través de API...')
...
def validar_credenciales(correo,password):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        query = "SELECT nombre, alias FROM usuarios WHERE clave = %s AND correo = %s  AND estado = 'ACTIVO'"
        print( query )
        print( correo )
        print( password )
        cur.execute(query, (password.strip(), correo.strip(),))
...
def obtener_datos_db(categoria, nivel, tipos_sel=None, subtipos_sel=None):
    ...
    try:
        ...
        #query = f"SELECT * FROM public.palabras "
...
def api_login():
    ...
    usuario = validar_credenciales(correo,password)
    if usuario:
...
<script>
    ...
    <div class="container">
        ...
        <!-- Login Form -->
        <div id="loginForm">
            <input type="input" id="correo" placeholder="Correo Electronico" required>
            <br>
    ...
    <input type="hidden" id="cuerpo1" value="
    Hola {nombre},
¡Gracias por registrarte! Para completar tu perfil y crear tu contraseña, 
haz clic en el siguiente enlace:
{enlace}
...
//            alert(urlActual);
            ...
            try {
                const response = await fetch( urlActual + 'api/login', {
                    method: 'POST',
                ...
                if (data.success) {
                    window.location.href = urlActual + 'dashboard';
                } else {
        ...
        async function registro() {
            ...
            const data1 = {
                destino: document.getElementById('regCorreo').value,
                asunto: 'Información',
                cuerpo: document.getElementById('cuerpo1').value
            };
            ...
            try {
                console.log('fetch https://systemsya.com/diccionario/api/registro');
                const response = await fetch(urlActual+'api/registro', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                ...
                console.log('fetch https://systemsya.com/correo/index.php');                
                const response1 = await fetch('https://systemsya.com/correo/index.php?destino=aquinnobjesusg@gmail.com&asunto=AsuntoDelCorreo&cuerpo=CuerpoDelCorreo', {
                    method: 'GET'
                });
        ...
        async function recuperar() {
            ...
            try {
                const response = await fetch(urlActual+'api/recuperar', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
...
<script>
    ...
    <script>
        ...
        async function crearPassword() {
            ...
            try {
             const urlActual = window.location.href;
                const response = await fetch(urlActual+'api/crear-password', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({
                        token: token,
...
<script>
        ...
        <div class="sidebar">
            ...
            <div class="grid-2">
                ...
                <div class="form-group">
                    ...
                    <select id="categoria" onclick="seleccionTipos()">
                ...
                <div class="form-group" id="divtipos">
                    <label>Tipos (solo Palabras):</label>
                    <select id="tipos" multiple size="7" >
                    ...
                    <option value="4">Pronombre</option>
                    <option value="7">Preposición</option>
                    <option value="8">Conjunción</option>
                    <option value="3">Determinante</option>
                    <option value="9">Interjección</option>
                    </select>
                </div>
                <div class="form-group" id="divsubtipos" style="display: none">
    ...
    <script>
        /**
         * rutina_audio - Versión JavaScript/HTML
         * 
         ...
         *     except: pass
         * 
         * Equivalente en JavaScript usando Web Speech API (soporte nativo en navegadores modernos)
         */
        ...
        class AudioProcessor {
            ...
            /**
             * Convierte el código de idioma de gTTS (2 letras) a formato BCP 47
             * @param {string} lang - Código de idioma (ej: 'en', 'es', 'fr')
             * @returns {string} - Código BCP 47 completo
             */
            ...
            /**
             * Intenta encontrar una voz específica para el idioma
             * @param {string} langCode - Código de idioma BCP 47
             * @returns {SpeechSynthesisVoice|null}
             */
            findVoice(langCode) {
                const voices = this.synth.getVoices();
                // Primero intentamos encontrar voz exacta
                let voice = voices.find(v => v.lang === langCode);
                // Si no, buscamos voz que empiece con el código
                if (!voice) {
            ...
            /**
             * Función principal: reproduce texto como audio (similar a gTTS)
             * @param {string} texto - El texto a reproducir
             * @param {string} lang - Código de idioma (2 letras: 'en', 'es', etc.)
             * @returns {Promise<void>}
             */
            async speak(texto, lang) {
                if (!texto || texto.trim() === '') {
                    console.warn('Texto vacío, no se reproduce audio');
                    this.showStatus('⚠️ No hay texto para reproducir', 'error');
                    return;
                }
                ...
                this.utterance.onstart = () => {
                    ...
                    this.showStatus(`🔊 Reproduciendo... (${lang})`, 'loading');
                };
                
                this.utterance.onend = () => {
                    this.isSpeaking = false;
                    this.showStatus('✅ Reproducción completada', 'success');
                ...
                this.utterance.onerror = (event) => {
                    ...
                    this.showStatus('❌ Error al reproducir audio', 'error');
                };
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            speakBtn.addEventListener('click', () => {
                ...
                if (!texto) {
                    audioProcessor.showStatus('✏️ Escribe algún texto primero', 'error');
                    return;
                }
                
                audioProcessor.speak(texto, lang);
            });
            
            // Atajo: Enter en el textarea (Ctrl+Enter o Cmd+Enter)
        ...
        // Función auxiliar para ejemplos
        function setExample(text) {
            document.getElementById('textInput').value = text;
            document.getElementById('speakBtn').click();
        }
        
        // Exportar al scope global para uso desde consola
        window.rutinaAudio = (texto, lang = 'en') => {
            const processor = new AudioProcessor();
            return processor.speak(texto, lang);
        };
        
        function speakIs( texto, lang ) {
...
//        console.log('✅ rutina_audio disponible globalmente como window.rutinaAudio(texto, lang)');
    </script>    
...
//            alert( document.getElementById('categoria').value );
            ...
            const elemento1 = document.getElementById('divtipos');
            const elemento2 = document.getElementById('divsubtipos');
            const elemento3 = document.getElementById('divmenu');

        if ( elemento.value == "Palabras" ) {
            elemento1.style.display = 'block';
            elemento2.style.display = 'none';
            elemento3.style.display = 'none';
        }

        if ( elemento.value == "Modismos" ) {
            elemento1.style.display = 'none';
            elemento2.style.display = 'none';
        }

        if ( elemento.value == "Verbos Compuestos" ) {
            elemento1.style.display = 'none';
            elemento2.style.display = 'none';
        }
        ...
        async function cargarDatos() {
            ...
            const elementoM = document.getElementById('divmenu');
            elementoM.style.display = 'none';
            try {
                const urlActual = window.location.href;
                
                const response = await fetch('https://systemsya.com/diccionario/api/cargar-datos', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify(data)
                });
                ...
                if (result.success) {
                    ...
                    elementoM.style.display = 'block';
                    renderizarModo();
                } else {
                    mostrarFeedback('Seleccione un Tipo de Palabra', 'error');
                }
            } catch (error) {
            ...
            palabras.forEach((p, i) => {
                html += `
                    <div class="word-card">
                        ...
                        <!-- <button onclick="reproducirAudio('${p.destino}', 'en')">🎯 </button> -->
                    </div>
                `;
            });
            html += '</div>';
        ...
        async function reproducirAudio(texto, lang) {
            lang = document.getElementById('idiomaOri').value;
            try {
                speakIs( texto, lang );
        ...
        async function logout() {
            const urlActual = window.location.href;
            await fetch(urlActual+'api/logout', { method: 'POST' });
            window.location.href = 'https://systemsya.com/diccionario/';
        }
    </script>

# =============================================================================
# DICCIONARIO INTERACTIVO - VERSIÓN PARA WINDOWS
# Archivo: simularvistabil08.py
# =============================================================================
...
# Configurar asyncio para Windows (evita errores de event loop)
...
def obtener_ruta_segura(ruta_relativa):
    """Convierte rutas para que funcionen en Windows y Linux."""
    if platform.system() == "Windows":
        return str(Path(ruta_relativa))
    else:
        return ruta_relativa

def verificar_dependencias_windows():
    """Verifica que las dependencias estén instaladas en Windows."""
    try:
        if sys.platform == "win32":
            # Verificar que pyaudio esté instalado
            try:
                import pyaudio
            except ImportError:
                st.warning("⚠️ Advertencia: pyaudio no está instalado.")
                st.info("📌 Para instalar: pip install pyaudio")
                st.info("📌 Si falla, descarga desde: https://www.lfd.uci.edu/~gohlke/pythonlibs/#pyaudio")
    except:
        pass
...
def aplicar_configuracion_estetica():
    ...
    st.set_page_config(
        page_title="Diccionario Interactivo", 
        layout="wide",
...
def procesar_voz_laboratorio(idioma_code):
    ...
    try:
        ...
        st.error("❌ Pyaudio no está instalado. La funcionalidad de voz no está disponible.")
        st.info("📌 En Windows, instala con: pip install pyaudio")
        return None
...
# =============================================================================
# BLOQUE 6: EL MOTOR DEL APLICATIVO (LOGICA DE MENÚS Y AUTO-ENTRENA)
# =============================================================================
...
def ejecutar_aplicativo_principal():
    ...
    # --- 1. Inicialización de Estado Interno ---
    ...
    def obtener_par(idx, lista, col, ori):
        ...
        palabra_ing = reg1[col] if reg1[1] == 1 else reg2[col]
        palabra_esp = reg1[col] if reg1[1] == 2 else reg2[col]
    ...
    def rutina_cargar_db(categoria, nivel, tipos_sel, subtipos_sel=None):
        try:
            conn = psycopg2.connect(host="localhost", database="pprueba", user="postgres", password="peluche01", port="5432")
            cur = conn.cursor()
            mapeo = {
    ...
    with st.sidebar:
        ...
        if not st.session_state.jugando:
            habla = st.selectbox("Idioma Origen:", ["Español", "Inglés"], key="main_panel_idioma_origen")
            ...
            if cat == "Palabras":
                ...
                selectores_seleccionados = st.multiselect("Tipos:", list(opciones.keys()), default=["Sustantivo"], key="main_panel_multiselect_tipos")
                t_sel = [opciones[s] for s in selectores_seleccionados]
                
                if "Verbos Regulares" in selectores_seleccionados or "Verbos Irregulares" in selectores_seleccionados:
                    tiempos = {
                        "Forma Base / Presente (1)": 1,
                        "Pasado Simple (2)": 2,
                        "Participio Pasado (3)": 3
                    }
                    subtipos_v = [tiempos[stmp] for stmp in st.multiselect(
                        "Tiempos Verbales:", 
                        list(tiempos.keys()), 
                        default=["Forma Base / Presente (1)"],
                        key="main_panel_tiempos_verbales"
                    )]
            
            niv_sel = st.radio("NIVEL:", ["NIVEL 1", "NIVEL 2", "NIVEL 3"], horizontal=True, key="main_panel_radio_nivel")
            nivel_num = int(niv_sel.split()[-1])
            
            if st.button("🔄 CARGAR / ACTUALIZAR NUEVA DATA", use_container_width=True):
                res, col = rutina_cargar_db(cat, nivel_num, t_sel, subtipos_v)
                st.session_state.update({
            ...
            if st.session_state.total_p > 0:
                if st.session_state.modo_falla_activo:
                    ...
                    max_permitido = max(1, st.session_state.total_p)
                    
                    if st.session_state.r_ini > max_permitido:
                        st.session_state.r_ini = max_permitido
                        
                    st.session_state.r_ini = st.number_input("Desde:", min_value=1, max_value=max_permitido, value=st.session_state.r_ini)
                    st.session_state.r_fin = st.number_input("Hasta:", min_value=1, max_value=max_permitido, value=st.session_state.r_fin)
                    st.markdown("---")
            ...
            if st.button("📊 VER FALLAS + PENDIENTES"):
                st.session_state.viendo_resumen = True
                st.rerun()
            if st.button("❌ ABORTAR JUEGO"):
                st.session_state.jugando = False
                st.rerun()

    # --- 4. Lógica de Navegación y Pantallas ---
    if st.session_state.get('modo_falla_activo', False):
        current_subset = st.session_state.get('subset_fallas', [])
        total_int = len(current_subset) // 2
        st.info(f"📚 Modo Refuerzo: Repasando tus {total_int} fallas.")
    elif st.session_state.data:
        current_subset = st.session_state.data[(st.session_state.r_ini-1)*2 : st.session_state.r_fin*2]
        total_int = len(current_subset) // 2
    else:
        current_subset = []
        total_int = 0
    ...
    if current_subset:
        if not st.session_state.jugando and not st.session_state.viendo_resumen:
            ...
            with c1:
                st.button("📋 REPASO", key="btn_m1", on_click=lambda: st.session_state.update({'menu_actual': "REPASO"}), use_container_width=True)
            with c2:
                st.button("🎧 ENTRENA", key="btn_m2", on_click=lambda: st.session_state.update({'menu_actual': "ENTRENA"}), use_container_width=True)
            with c3:
                st.button("🎯 JUEGO", key="btn_m3", on_click=lambda: st.session_state.update({'menu_actual': "JUEGO"}), use_container_width=True)
        ...
        if st.session_state.viendo_resumen:
            ...
            if st.button("🏠 REFORZAR ESTO (IR A REPASO)", use_container_width=True):
                ...
                for p, info in st.session_state.j_fallidos_dict.items():
                    ...
                    if st.session_state.idioma_ori == "Español":
                        f_esp[st.session_state.col_idx] = p
                        f_ing[st.session_state.col_idx] = info['traduccion']
                    else:
                        f_ing[st.session_state.col_idx] = p
                        f_esp[st.session_state.col_idx] = info['traduccion']
                    
                    nueva_data.extend([f_ing, f_esp])
                ...
                if pends:
                    ...
                    with col_iz:
                        if st.session_state.j_paso == 1:
                            ...
                            """, unsafe_allow_html=True)
                            if st.session_state.j_status == "OK":
                                st.balloons()
                            else:
                                st.snow()
                    ...
                    with st.container(height=300):
                        ...
                        if st.session_state.j_fallidos_dict:
                            ...
                            st.success("¡Excelente! No hubo fallas en esta sesión.")
                    ...
                    with c1:
                        if st.button("🏠 REFORZAR FALLAS", use_container_width=True):
                            ...
                            st.session_state.menu_actual = "REPASO"
                            st.session_state.jugando = False
                            st.rerun()
                    with c2:
            ...
            for i in range(0, len(current_subset), 2):
                ...
                with c_txt:
                    st.markdown(f"**{i//2 + 1}. {v_i} ➔ {v_d}**")
                with c_aud:
                    st.button("🔊", key=f"ra_{i}", on_click=rutina_audio, args=(v_d, v_l))
                with c_go:
                    if st.button("🎯", key=f"rg_{i}"):
                        st.session_state.update({
                            'menu_actual': "ENTRENA",
                            'idx_entrena': i,
                            'ent_step': 0,
            ...
            if not st.session_state.auto_entrena:
                ...
                with c_de:
                    ...
                    if st.button("SIGUIENTE ➔", key="btn_sig_ent"):
                        if st.session_state.ent_step == 0:
                            st.session_state.ent_step = 1
                        else: 
                            st.session_state.idx_entrena = (st.session_state.idx_entrena + 2) % len(current_subset)
                            st.session_state.ent_step = 0
                        st.rerun()
                    if st.button("🔊 OÍR", key="btn_aud_ent"):
                        rutina_audio(v_s, v_l)
                    st.markdown("---")
                    if st.button("🚀 MODO AUTO", key="btn_auto_on"):
                        st.session_state.auto_entrena = True
                        st.rerun()
            else:
                st.info("Modo Automático Activo: Relájate y escucha.")
                placeholder = st.empty()
                if st.button("🛑 DETENER", key="btn_auto_off"):
...
# =============================================================================
# INICIALIZACIÓN DE SESIÓN (ESTADO GLOBAL)
# =============================================================================
...
def main():
    ...
    # Cambiar al directorio del script para evitar problemas de rutas en Windows
    os.chdir(os.path.dirname(os.path.abspath(__file__)))
    
    # Verificar dependencias en Windows
    verificar_dependencias_windows()
    
    # Aplicar configuración estética
    aplicar_configuracion_estetica()
    
    # Inicializar estado
    inicializar_estado_aplicativo()
    
    # Verificar parámetros de URL (token de activación/recuperación)

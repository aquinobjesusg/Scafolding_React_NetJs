...
def proceso_solicitar_recuperacion(correo):
    ...
    try:
        ...
        conn = psycopg2.connect(host="localhost", database="systemsy_dicc1", user="systemsy_dicc", password="systemsy_dicc", port="5432")
        cur = conn.cursor()
...
def registrar_usuario_inicial(nombre, alias, correo, nivel_texto):
    ...
    try:
        ...
        conn = psycopg2.connect(
            host="localhost", database="systemsy_dicc1", user="systemsy_dicc", password="systemsy_dicc", port="5432"
        )
        cur = conn.cursor()
...
def ejecutar_aplicativo_principal():
    # 🔑 ESCUDO DE PROTECCIÓN: Evita pantallas en blanco o renderizados antes del login
    if not st.session_state.get('auth', False):
        return

    # --- Lógica de recuperación por fallas ---
    ...
    def obtener_par(idx, lista, col, ori):
        ...
        # Buscamos cuál es inglés y cuál es español usando el id_lenguaje (columna índice 1)
        palabra_ing = reg1[col] if reg1[1] == 1 else reg2[col]
        palabra_esp = reg1[col] if reg1[1] == 2 else reg2[col]
        
        # Ahora decides qué mostrar según el "Idioma Origen" elegido en la interfaz
        if ori == "Español":
            return palabra_esp, palabra_ing, 'en', ""  # Pregunta en Español, respondes en Inglés
        else:
            return palabra_ing, palabra_esp, 'es', ""  # Pregunta en Inglés, respondes en Español
    ...
    def rutina_cargar_db(categoria, nivel, tipos_sel, subtipos_sel=None):
        try:
            ...
            # Filtro por Tipo de Palabra (Sustantivo, Verbo, etc.)
            if col_tipo and tipos_sel:
                query += f"AND {col_tipo} IN ({','.join(['%s']*len(tipos_sel))}) "
                params.extend(tipos_sel)
                
                # Filtro por Subtipo (Tiempos Verbales) exclusivo para la tabla Palabras
    ...
    if st.session_state.get('modo_falla_activo', False):
        ...
        # Mostramos el aviso flotante aquí de forma segura
    ...
    # Esto asegura que solo se muestren las pantallas si realmente hay registros asignados
    if current_subset:
        ...
        if st.session_state.viendo_resumen:
            ...
            # 🛠️ INTERVENCIÓN QUIRÚRGICA AQUÍ: Asignación limpia de IDs de idioma para las fallas
            if st.button("🏠 REFORZAR ESTO (IR A REPASO)", use_container_width=True):
                ...
                for p, info in st.session_state.j_fallidos_dict.items():
                    ...
                    # Seteamos obligatoriamente los IDs en la posición [1]
                    f_ing[1] = 1  # 1 es ID de Inglés
                    f_esp[1] = 2  # 2 es ID de Español
                    
                    # Ubicamos las palabras en el índice dinámico correspondiente de la base de datos
                    ...
                    # Extendemos la lista respetando el orden estructural esperado
...
# =============================================================================
# FUNCIONES AUXILIARES (STUBS REQUERIDOS)
# =============================================================================

def convertir_mic_a_texto(audio_input):
    r = sr.Recognizer()
    try:
        with sr.AudioFile(audio_input) as source:
            audio_data = r.record(source)
            texto = r.recognize_google(audio_data, language="en-US" if st.session_state.idioma_ori == "Español" else "es-ES")
            return texto
    except:
        return ""
...
# =============================================================================
# PUNTO DE ENTRADA (CONTROLADOR PRINCIPAL)
# =============================================================================

...
export const DynamicReports: React.FC = () => {
  ...
  const categoryStats = [
    { name: 'Desarrollo Web', estudiantes: 16040, cursos: 2, color: '#4F46E5' },
    { name: 'Inteligencia Artificial', estudiantes: 6200, cursos: 1, color: '#7C3AED' },
    { name: 'Diseño UI/UX', estudiantes: 4300, cursos: 1, color: '#06B6D4' },
    { name: 'Cloud & DevOps', estudiantes: 3100, cursos: 1, color: '#10B981' },
    { name: 'Bases de Datos', estudiantes: 2200, cursos: 1, color: '#F59E0B' },
  ];

  // User role breakdown
  const roleDistribution = [
    { name: 'Estudiantes', value: users.filter((u) => u.role === 'STUDENT').length || 28, color: '#10B981' },
    { name: 'Instructores', value: users.filter((u) => u.role === 'INSTRUCTOR').length || 4, color: '#F59E0B' },
    { name: 'Editores', value: users.filter((u) => u.role === 'EDITOR').length || 2, color: '#3B82F6' },
    { name: 'Administradores', value: users.filter((u) => u.role === 'ADMIN').length || 1, color: '#EF4444' },
  ];
  ...
  const coursePerformance = courses.map((c) => ({
    ...
    precio: c.price,
    ingresosEstimados: Math.round(c.studentsCount * c.price * 0.85),
  }));
  ...
  const handleExportFullReport = () => {
    const reportData = courses.map((c) => ({
      ...
      AlumnosActivos: c.studentsCount,
      CalificaciónEstrellas: c.rating,
      PrecioUSD: c.price,
      IngresosBrutosUSD: (c.studentsCount * c.price).toFixed(2),
      Estado: c.status,
    }));
    ...
    exportToExcel(
      ...
      [
        ...
        { header: 'Alumnos Activos', key: 'AlumnosActivos' },
        { header: 'Calificación Promedio', key: 'CalificaciónEstrellas' },
        { header: 'Precio (USD)', key: 'PrecioUSD' },
        { header: 'Ingresos Brutos Estimados ($)', key: 'IngresosBrutosUSD' },
        { header: 'Estado del Curso', key: 'Estado' },
      ],
      'informe_analiticas_dinamicas',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-base font-bold text-gray-900 tracking-tight">
            Reportes Dinámicos & Inteligencia de Negocio
          </h2>
          <p className="text-xs text-gray-500">
            Analiza el comportamiento de ventas, retención de estudiantes y rendimiento del catálogo.
          </p>
        </div>

        <div className="flex items-center gap-2">
          {/* Time range switcher */}
          <div className="flex rounded-xl bg-white border border-gray-200 p-1 shadow-2xs">
            <button
              onClick={() => setDateRange('30d')}
              className={`rounded-lg px-2.5 py-1 text-xs font-semibold transition-all ${
                dateRange === '30d' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
              Últimos 30 días
            </button>
            <button
              ...
              className={`rounded-lg px-2.5 py-1 text-xs font-semibold transition-all ${
                dateRange === '90d' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
              Trimestre
            </button>
            <button
              ...
              className={`rounded-lg px-2.5 py-1 text-xs font-semibold transition-all ${
                dateRange === 'year' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
              Año Actual
            </button>
        ...
        <div className="flex items-center justify-between border-b border-gray-100 pb-4 mb-4">
          ...
          <div className="flex items-center gap-4 text-xs font-semibold">
            <span className="flex items-center gap-1.5 text-indigo-600">
              <span className="h-2.5 w-2.5 rounded-full bg-indigo-600" /> Ingresos ($ USD)
            </span>
        ...
        <div className="h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={revenueTrendData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              />
              <Area type="monotone" dataKey="ingresos" stroke="#4F46E5" strokeWidth={2.5} fillOpacity={1} fill="url(#colorIngresos)" />
